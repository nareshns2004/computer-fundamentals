...
main(int argc, char **argv){
    ...
    student_t *naresh = xcalloc(object_db, "student_t", 1);
    mld_set_dynamic_object_as_root(object_db, naresh);

    student_t *priyanka = xcalloc(object_db, "student_t", 1);
    strncpy(priyanka->stud_name, "priyanka", strlen("priyanka"));
    //naresh->best_colleage = priyanka;
